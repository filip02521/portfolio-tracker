...
class PDFReportGenerator:
    ...
    def generate_tax_report_pdf(self, year: int, transactions_data: list, realized_pnl: float = None, filename: str = None):
        """Generate comprehensive tax report PDF - returns bytes in memory"""
        ...
        total_buys = len([t for t in transactions_data if t.get('type', '').lower() == 'buy'])
        total_sells = len([t for t in transactions_data if t.get('type', '').lower() == 'sell'])
        
        # Use provided realized PNL or calculate from transactions
        if realized_pnl is None:
            realized_pnl = 0
            for tx in transactions_data:
                if tx.get('type', '').lower() == 'sell':
                    # Use realized_pnl if available, otherwise calculate from price difference
        ...
        for tx in transactions_data[:50]:  # Limit to first 50 transactions for PDF
            ...
            tx_type_str = tx.get('type', '').lower()
            tx_type = 'Kupno' if tx_type_str == 'buy' else 'Sprzedaż' if tx_type_str == 'sell' else 'N/A'
    ...
    def generate_portfolio_summary_pdf(self, portfolio_data: dict):
        """Generate portfolio summary PDF - returns bytes in memory"""
        ...
        info_text = f"Wygenerowano: {report_date}<br/>Wartość portfolio: ${total_value:,.2f}"
        info = Paragraph(info_text, self.styles['CustomBody'])
        story.append(info)
        story.append(Spacer(1, 20))
