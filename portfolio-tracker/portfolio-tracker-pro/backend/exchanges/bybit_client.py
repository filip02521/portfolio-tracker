...
class BybitClient:
    ...
    def __init__(self, api_key: Optional[str] = None, secret_key: Optional[str] = None):
        ...
        self.secret_key = secret_key or Config.BYBIT_SECRET_KEY
    ...
    def _make_request_with_retry(self, func, max_retries=3, base_delay=1):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt)
                    print(f"Bybit API timeout (read timeout), retrying in {delay:.1f} seconds... (attempt {attempt + 1}/{max_retries})")
                    time.sleep(delay)
                    continue
                else:
                    print(f"Max retries reached for timeout error: {e}")
                    raise ConnectionError(f"Bybit API timeout after {max_retries} retries: {error_msg}")
            except ConnectionError as e:
                error_msg = str(e)
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt)
                    print(f"Bybit API connection error, retrying in {delay:.1f} seconds... (attempt {attempt + 1}/{max_retries})")
                    time.sleep(delay)
                    continue
                else:
                    print(f"Max retries reached for connection error: {e}")
                    raise ConnectionError(f"Bybit API connection failed after {max_retries} retries: {error_msg}")
            except RequestException as e:
                error_msg = str(e)
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt)
                    print(f"Bybit API request error, retrying in {delay:.1f} seconds... (attempt {attempt + 1}/{max_retries})")
                    time.sleep(delay)
                    continue
                else:
                    print(f"Max retries reached for request error: {e}")
                    raise RequestException(f"Bybit API request failed after {max_retries} retries: {error_msg}")
            except Exception as e:
                error_msg = str(e)
                error_type = type(e).__name__
                print(f"Unexpected error in Bybit API request ({error_type}): {error_msg}")
                if attempt < max_retries - 1:
                    delay = base_delay * (2 ** attempt)
                    ...
                    raise Exception(f"Bybit API error after {max_retries} retries ({error_type}): {error_msg}")
